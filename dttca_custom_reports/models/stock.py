...
class StockMove(models.Model):
    ...
    def _get_lots(self):
        ...
        if len(result) > 1:
            my_result = []
            my_result_without_date = []
            for r in result:
                if r.get("date_row", False):
                    my_result.append(r)
                else:
                    my_result_without_date.append(r)
            if my_result:
                my_result = sorted(my_result, key=lambda x: x["date_row"])
                if my_result_without_date:
                    my_result += my_result_without_date
                result = my_result
        return result
